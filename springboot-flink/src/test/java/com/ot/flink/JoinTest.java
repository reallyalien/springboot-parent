...
//        env.setParallelism();
        ...
        Table eventTable2 = tableEnvironment.fromDataStream(source2, $("user").as("u2"), $("timestamp").as("t2"), $("url").as("url2"));

        tableEnvironment.createTemporaryView("a", eventTable1);
        tableEnvironment.createTemporaryView("b", eventTable2);

//        tableEnvironment.toDataStream(eventTable1).print("11111111");
//        tableEnvironment.toDataStream(eventTable2).print("22222222");

//        //执行sql
        String sql = "select a.u1,b.u2,a.url1,a.t1,b.url2,b.t2 from a left join  b on a.u1=b.u2 ";
        Table result = tableEnvironment.sqlQuery(sql);


        //将表结果转换成stream输出
        DataStream<Row> rowDataStream = tableEnvironment.toChangelogStream(result);
//        DataStream<Row> rowDataStream = tableEnvironment.toDataStream(result);
        rowDataStream.print();
//
        env.execute();
    }
...
//        env.setParallelism();
        ...
        SingleOutputStreamOperator<Row> process = source.process(new ProcessFunction<Map, Row>() {
            @Override
            public void processElement(Map value, ProcessFunction<Map, Row>.Context ctx, Collector<Row> out) throws Exception {

...
public class ProduceCsv {
    ...
    private static final String topicName = "realtime-dev-source-csv-001";
        ...
        try {
            ...
            for (long i = 1; i <= 5000; i++) {
                ...
                String s = String.format(format,
                        user.getUserId(), user.getUserName(), user.getAddress(),
                        user.getBirthday(), user.getDeath(), user.getLongDeath(), user.getAge(), user.getWeight(),
                        user.getHeight(), user.getPrice(), user.getAdult()
                );
